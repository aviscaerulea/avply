...
- 公開関数 `changePlaybackRate` / `changeVolume` / `cycleNormalize` / `cycleVoiceClarity` の末尾で `m_gResetActive = false` を立てる
...
Off ↔ ON 切替時は 50ms のゲインランプで段差を回避する。
...
| パラメータ | 共通 | 小 | 中 | 大 |
|-----------|------|----|----|----|
| Threshold | - | -20.0 dB | -25.0 dB | -30.0 dB |
| Makeup gain | - | +5.0 dB | +10.0 dB | +13.0 dB |
| Ratio | 6:1 | - | - | - |
| Attack | 20 ms | - | - | - |
| Release | 250 ms | - | - | - |
...
threshold は -60.0〜0.0 dBFS、makeup は 0.0〜24.0 dB にクランプする。
