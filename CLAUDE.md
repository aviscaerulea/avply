...
再生時の RMS コンプレッサによる音量差縮小機能。大声と小声のレベル差をリアルタイムに圧縮する。

- デフォルト：ON（`Settings::normalizeEnabled` の既定値 `true`）
- トグル：`N` キー、または右クリック →「設定」→「ノーマライズで音量差を縮小する」
- 状態表示：ステータスバーに「🎚 ノーマライズ」（ON 時のみ）
- 永続化：レジストリ（`HKEY_CURRENT_USER\Software\avply\avply\normalizeEnabled`）

ON↔OFF 切替時は 50ms のゲインランプで段差を回避する。

DSP パラメータ（`Normalizer.cpp` の constexpr 定数）：
...
**audio thread underrun 対策**

1.0x 超の再生速度とノーマライズ ON の組み合わせで、`QAudioSink::write` が間に合わず書き込みアンダーラン（プチプチノイズ）が発生する問題がある。以下の対策を施している。
...
ノーマライズ OFF にすれば underrun は軽減するが、対策後は ON + 2.0x でも無音欠落は発生しない。

### サイレンストーン設定

`avply.toml` の `[audio]` セクションでサイレンストーン（BT アイドル復帰時のプチノイズ抑制用、常時不可聴トーン出力）を制御する。

- `silence_tone_enabled`: ON/OFF（既定 `true`）。`false` で `SilenceTone` を起動せず OS への常時音声出力を完全に止める。スピーカー環境や有線 DAC 環境ではユーザ判断で OFF にできる
- `silence_tone_freq_hz`: トーン周波数（既定 1000.0、20〜20000 Hz にクランプ）。1 kHz は SBC 等 BT コーデックの確実なパスバンド内で、コーデックを「アクティブ」状態に保てる
