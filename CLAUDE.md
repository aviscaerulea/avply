...
- 入力が MP4 コンテナで受け入れ可能なら何でも実行可（不可なら ffmpeg がエラーで返す）
...
入力ファイルと同一フォルダに `<元ファイルベース名>_clip.mp4` で出力する。
同名が既に存在する場合は `_clip_2.mp4`、`_clip_3.mp4` の順で衝突回避する。
変換・トリムどちらも同じサフィックスを使用する。
