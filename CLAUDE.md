...
現在は `VideoView::setVolume` が `qBound(0.0, volume, 1.0)` で音量を 0.0〜1.0 へクランプしてから `AudioWorker` へ渡すため、同問題を回避している。
...
- 状態は ON/OFF の 2 値。切替操作は `C` キーのみ（トグル。C は Clarity の頭文字。旧 N キーから直観性のため変更）
...
- `headroom_db = 6` / `initial_gain_db = 6`：headroom は full-scale から差し引いた値が AGC2 の出力ターゲットになる。小さいほどターゲットが上がり小声を強く持ち上げる。大声は既にターゲット以上のため影響を受けず、クリップ耐性もプリアッテネーションで担保されるため変わらない。当初はクリップフレーム 0 を実測できた 4dB を採用した。しかし ON 時の全体レベルがわずかに高いとの聴感評価により 6dB へ緩めた。（持ち上げ量 -2dB。ターゲットを下げる方向のためクリップ耐性は安全側に働く）`initial_gain_db` は既定 15dB から控えめにして再生直後の過大ブーストを抑える
