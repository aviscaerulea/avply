...
再生時の RMS コンプレッサによる音量差縮小機能。大声と小声のレベル差を圧縮する。
シーク・ファイル切替直後の `kRmsWindowMs`（10ms）は RMS 追跡器の収束を待つため `currentGain=1.0` で素通りし、その後に圧縮を開始する（warmup）。シーク直後のポップノイズ抑制が目的だ。
