...
`avply.toml` の `[audio].volume` で再生音量（倍率）を指定する。
デフォルトは 1.00、範囲は 0.00〜3.00 にクランプする。

`QMediaPlayer` の `QAudioOutput` は内部で 0.0〜1.0 にクランプされるため、
1.00 超を実現するには `QAudioBufferOutput` でサンプルを取得し、
gain を線形乗算したうえで `QAudioSink` に push する構成を取る（`VideoView` が担う）。
ピーク超過分は `tanhf` で ±1.0 付近に滑らかに飽和させる（ソフトクリップ）。
ハードクリップは高調波歪み（ザリザリ音）を生むため避ける。
特に playbackRate ≠ 1.0 時の resampler は overshoot サンプル（±1.0 超）を生成するため、gain 1.0 でもこの経路を通る。
会議録など音量が小さい素材の聴取性を上げる用途を想定する。
