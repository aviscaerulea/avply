...
- 強度は 4 段階：Off / 小 / 中 / 大（デフォルト：中。`Settings::speechEnhanceLevel` の既定値 `2`）
- 切替操作：`N` キーのみ。押下する度に Off → 小 → 中 → 大 → Off の順で循環する
- 状態表示：ステータスバーに「Voice:0〜3」を常時表示する（0=Off / 1=小 / 2=中 / 3=大）
- 永続化：レジストリ（`HKEY_CURRENT_USER\Software\avply\avply\speechEnhanceLevel`、int 値 0〜3）
