...
// 2 段目：Peaking EQ
// 3kHz 中心の子音帯域（プレゼンス）を +5dB 持ち上げてこもり感を軽減する
static constexpr float kPeakFreq = 3000.0f;
static constexpr float kPeakQ    = 1.0f;
static constexpr float kPeakDb   = +5.0f;

// 3 段目：High-shelf
// 8kHz 以上を +2dB シェルフブーストして高域の抜けを補い、明瞭感を底上げする
static constexpr float kShelfFreq = 8000.0f;
static constexpr float kShelfDb   = +2.0f;
...
VoiceClarity::VoiceClarity(int sampleRate, int channels, bool initialEnabled)
    ...
    , m_applyRatio(initialEnabled ? 1.0f : 0.0f)
{
    ...
    const float rate = static_cast<float>(sampleRate > 0 ? sampleRate : 48000);
    m_rampStep = 1.0f / (rate * kRampMs * 0.001f);

    m_coeffs[0] = makeHpfCoeff      (rate, kHpfFreq,   kHpfQ);
    m_coeffs[1] = makePeakCoeff     (rate, kPeakFreq,  kPeakQ,  kPeakDb);
    m_coeffs[2] = makeHighShelfCoeff(rate, kShelfFreq, kShelfQ, kShelfDb);
}

void VoiceClarity::setEnabled(bool enabled)
{
    m_enabled = enabled;
}
...
void VoiceClarity::process(float* samples, std::ptrdiff_t n)
{
    ...
    if (!m_enabled && m_applyRatio == 0.0f) {
        return;
    }
