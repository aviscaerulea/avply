...
- **音声明瞭化**  
Biquad EQ 3 段カスケード（HPF 100Hz + プレゼンスブースト 3kHz + 高域シェルフ 8kHz）で人声の子音帯域を持ち上げ、こもり気味の音声を聞き取りやすくする。強度は Off / 小 / 中 / 大 の 4 段階で、`V` キーを押下する度に Off → 小 → 中 → 大 → Off の順で循環する。Off 以外のときはステータスバーに「🎤 音声明瞭化 (小/中/大)」を表示する。設定はレジストリに保存され、デフォルトは中。
- **コンテキストメニュー**  
