...
強度は Off・標準・強の 3 段階で、N キーを押すたびに切り替わります。
...
- NVIDIA GPU（動画を変換する場合のみ必要。AV1 NVENC 対応、RTX 30 シリーズ以降を推奨）
...
手動で導入する場合は [Releases](https://github.com/aviscaerulea/avply/releases) から `avply-<version>-x64.zip` をダウンロードして展開し、`avply.exe` を起動します。
...
G キーは 1 回目で中立の状態（速度 1.00、音量 100%、音声強調 Off）へ、2 回目で起動時の設定値へ戻します。
画面下部には、現在の再生速度・音量・音声強調の強度を常に表示します。

### トリムと変換

トリムは区間を指定してから実行します。
指定した区間はシークバー上に赤く表示されます。
...
同じ名前が既にある場合は `_mod2`、`_mod3` と番号が付きます。
...
PC 固有の値をリポジトリの管理から外したい場合は、同じフォルダの `avply.local.toml` に同じキーを書くと後勝ちで上書きされます。
...
これらはレジストリに保存され、次回の起動でも維持されます。

## 制限事項

- トリムはキーフレーム単位で切り出すため、開始位置が指定より手前へ丸められる
...
ビルドは以下のコマンドで実行します。
実行ファイルは `out/Release/avply.exe` に生成されます。
