...
- **音声明瞭化**  
Biquad EQ 3 段カスケード（HPF 100Hz + プレゼンスブースト 3kHz +5dB + 高域シェルフ 8kHz +2dB）で人声の子音帯域を持ち上げ、こもり気味の音声を聞き取りやすくする。`V` キーまたは右クリック →「設定」で ON/OFF を切り替えられる。ON 時はステータスバーに「🎤 音声明瞭化」を表示する。設定はレジストリに保存され、デフォルトは ON。
- **コンテキストメニュー**  
右クリックでファイルを開く・変換・トリムを実行できる。設定サブメニューから再生中 topmost・シングルインスタンス強制・プロセス優先度・ノーマライズ・音声明瞭化を変更できる。（レジストリ保存）
...
3. シークバーのドラッグ、カーソルキー（← / →）、またはシークバー・プレビュー領域のマウスホイールで再生位置を移動する（デフォルト 5 秒スキップ）。カーソルキー（↑ / ↓）または Ctrl+マウスホイールで再生速度を ±0.05 倍ずつ調整、Shift+カーソルキー（↑ / ↓）または Shift+マウスホイールで音量を ±0.05 ずつ調整できる（現在速度・音量はステータスバーに表示）。`N` キーでノーマライズ（音量差縮小）、`V` キーで音声明瞭化（人声強調）の ON/OFF を切り替えられる。
