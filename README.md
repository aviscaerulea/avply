...
動画ファイルをシンプルに再生・トリミングできる Windows 11 GUI ツール。
...
- **高速トリム**：再エンコードせずキーフレーム単位で切り出す（「トリム」ボタン）。処理はディスクコピー速度近くだが、開始位置はキーフレームに丸められる
- **AV1 変換**：NVIDIA GPU（NVENC）を使用して AV1 + Opus 96kbps で出力（「変換」ボタン）
...
- NVIDIA GPU（「変換」機能を使う場合のみ必須。AV1 NVENC 対応 / RTX 30 シリーズ以降推奨）
...
6. 入力ファイルと同じフォルダに `<元ファイル名>_clip.mp4` として出力される
