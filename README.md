...
- **変換**：動画は NVIDIA GPU（NVENC）で AV1 + Opus 96kbps、音声は libopus 96kbps のみで出力する（「変換」ボタン）
...
5. 「変換」または「トリム」をクリックして処理を開始する（実行中は対応ボタンが「中止」に切り替わる）
    - 「変換」：動画は AV1 + Opus へ、音声は libopus 96kbps のみへ再エンコード
    - 「トリム」：再エンコードせず `-c copy` で高速切り出し（解像度・コーデック・ビットレートは入力をそのまま維持）
...
再生音量は `[audio].volume` で 1.00 を越えてブーストできる（既定 1.00、範囲 0.00〜3.00）。会議録など音声が小さい素材で有効である。
